...
> 这个 Fork 假设作者不会加入任何互联网词汇、游戏相关词库等等，并对这部分加以补全。

> 它是 [GrassBlock1/rime_config: 基于雾凇拼音的自用 rime 配置（补丁）](https://github.com/GrassBlock1/rime_config) 的精神延续。

> 它会是一个比较主观的配置，同时也会提供回退这些更改的说明。

# TODO
- [ ] 同步原有的个人配置
  - [ ] 添加颜文字候选（#463）
  - [ ] 输入法层面的中英文之间自动添加空格（#1174）
  - [ ] 语法模型（需要手动下载万象语法模型）
  - [ ] 双拼补丁（默认只给小鹤添加了，可以按需要修改成其它方案）
  - [ ] 自动纠错，若要使用直接取消掉注释即可
- [ ] 添加词库
  - [ ] Minecraft 相关词汇
  - [ ] 星露谷物语词汇
